  ...
  return (
    ...
    <div className="flex flex-col md:flex-row gap-12 lg:gap-16 w-full max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="w-full md:w-1/2">
        <h2 className="text-3xl font-bold mb-10 text-tagCred">Education</h2>
        <div className="relative">
          <div className="absolute top-5 left-5 w-1 h-full bg-gray z-0 rounded"></div>
          ...
          <div className="space-y-10">
            {educationElement.map((element) => (
              <div key={element.id} className="relative pl-14">
                ...
                <div className="bg-white border border-gray rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
                  <h3 className="text-xl font-bold text-red">{element.title}</h3>
                  <p className="text-sm text-dateCred mt-2">{element.date}</p>
                  <hr className="my-3 text-hrCred"/>
                  <p className="text-sm text-gpaCred font-medium">{element.GPA}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="w-full md:w-1/2">
        <h2 className="text-3xl font-bold mb-10 text-tagCred">Credentials</h2>
        <div className="relative">

          <div className="absolute top-5 left-5 w-1 h-full bg-gray z-0"></div>
          ...
          <div className="space-y-10">
            {certificationElement.map((element) => (
              <div key={element.id} className="relative pl-14">
                ...
                <div className="bg-white border border-gray rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
                  <h3 className="text-xl font-bold text-red">{element.Title}</h3>
                  <p className="text-md text-fromCred mt-1">{element.from}</p>
                  <div className="flex items-center justify-between">
                        ...
                        <a
                            ...
                            className="text-sm font-medium text-red hover:underline mt-4 inline-block group"
                        >
